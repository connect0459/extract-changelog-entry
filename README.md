...
- A fenced code block (` ``` ` or `~~~`) inside an entry is treated as literal content: `#`-prefixed lines, `## [` headings, and `---` dividers inside it are never promoted or treated as a section boundary. Fence and heading recognition are indentation-depth-agnostic — a heading or fence indented arbitrarily deep as a list item's continuation text (for example inside a two-level nested list, or a 10+-item ordered list) is still recognized, with a recognized heading shifting by the same rule as any other heading in the body, per above.
- A reference-style link `[X.Y.Z]: <url>` or `[X.Y.Z]: url` (with or without the angle-bracket wrapper) at the bottom of the file is used to build the "Full Changelog" link. If the link is a compare URL (`.../compare/vA...vB`), it's used directly; if it points at the tag itself (the first release has no prior tag to compare against), the link falls back to the tag's commit history. If no reference link exists for the version, no "Full Changelog" section is appended.
